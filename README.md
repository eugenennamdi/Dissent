...
- **Application API**: Synchronous `POST /api/research` plus explicit anonymous `POST /api/decisions`, with browser-local MVP persistence
...
# Explicit live application-route proof, including human decision recording
DEEPSEEK_API_KEY=your_key npm run test:integration:api
```
...
See [`docs/API_INTEGRATION.md`](./docs/API_INTEGRATION.md) for the frontend contract, browser-local persistence limitations, Vercel runtime requirement, and deployment rate-limit requirement.
