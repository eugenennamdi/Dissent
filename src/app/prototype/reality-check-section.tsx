...
export interface RealityCheckEditorialProps {
...
export function RealityCheckEditorialSection({
  assumptions,
  stressScenarios,
  ...
  // Primary tension assumption (prioritize QUESTIONED)
  ...
  const renderConditionCard = (
    ...
    return (
      <div
        key={cond.id}
        className="p-4 sm:p-5 rounded-xl bg-zinc-950/70 border border-zinc-800/90 shadow-2xs space-y-3.5 transition-colors hover:border-zinc-700"
      >
        ...
        {cond.type === 'QUANTITATIVE' ? (
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs sm:text-sm font-semibold text-zinc-100 leading-snug font-sans">
        ...
        <div className="border-l-2 border-zinc-700 pl-3 py-1 space-y-0.5 bg-zinc-900/40 rounded-r-md">
          <span className="font-mono text-[10px] uppercase tracking-wider text-zinc-400 font-medium block">
        ...
        {cond.type === 'QUANTITATIVE' ? (
          <div className="pt-2.5 border-t border-zinc-800/80 grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
            <div className="space-y-0.5">
              <span className="font-mono text-[10px] uppercase tracking-wider text-zinc-400 font-medium block">Target Metric</span>
              <span className="text-xs font-medium text-zinc-200 font-sans block truncate">{cond.targetMetric}</span>
            </div>
            <div className="space-y-0.5">
              <span className="font-mono text-[10px] uppercase tracking-wider text-zinc-400 font-medium block">Threshold</span>
              <span className="font-mono text-xs font-bold text-amber-400 block">{cond.triggerThreshold}</span>
            </div>
            <div className="space-y-0.5">
              <span className="font-mono text-[10px] uppercase tracking-wider text-zinc-400 font-medium block">Review Window</span>
              <span className="text-xs text-zinc-300 font-sans block">{cond.timeframe}</span>
            </div>
            <div className="space-y-0.5">
              <span className="font-mono text-[10px] uppercase tracking-wider text-zinc-400 font-medium block">Source</span>
              <span className="text-xs text-zinc-300 font-sans block">{cond.observableDataSource}</span>
            </div>
          </div>
        ) : (
          <div className="pt-2.5 border-t border-zinc-800/80 grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
            <div className="space-y-0.5">
              <span className="font-mono text-[10px] uppercase tracking-wider text-zinc-400 font-medium block">
                Verification Source
              </span>
        ...
        {hasEvidence && (
          <div className="pt-2.5 border-t border-zinc-800/80 flex flex-col sm:flex-row sm:items-baseline gap-2 text-xs">
            <span className="font-mono text-[10px] uppercase tracking-wider text-zinc-400 font-semibold shrink-0">
              Grounding Baseline:
            </span>
  ...
  return (
    <section className={`space-y-6 ${className}`}>
      {/* Section Header */}
      <div className="flex flex-col sm:flex-row sm:items-baseline justify-between gap-1 pb-1 border-b border-zinc-850">
        <div>
          <h2 className="text-lg font-semibold text-white tracking-tight">The Reality Check</h2>
      ...
      <div className="p-6 rounded-2xl bg-zinc-900/40 border border-zinc-800/80 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          <span className="font-mono text-xs text-zinc-400 bg-zinc-950 border border-zinc-800 px-2.5 py-0.5 rounded">
        ...
        <div className="space-y-2">
          <div
            ...
            className="h-2.5 w-full rounded-full overflow-hidden flex bg-zinc-950 border border-zinc-800"
          >
            {supportedPct > 0 && (
              <div
        ...
        {primaryTensionAssumption && (
          <div className="p-3.5 rounded-xl bg-zinc-950/60 border border-zinc-800/80 space-y-1.5 text-xs">
            <div className="flex items-center gap-2">
          ...
          {showAllAssumptions && (
            <div className="pt-3 space-y-3 border-t border-zinc-800/70 mt-3">
              {assumptions.map((asm) => (
                <div
                  key={asm.id}
                  className="p-3.5 rounded-xl bg-zinc-950/80 border border-zinc-800/90 space-y-2 text-xs"
                >
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <span className="text-zinc-200 font-medium leading-snug">
                  ...
                  {asm.challenge && (
                    <div className="text-zinc-400 leading-relaxed">
                      <span className="font-mono text-zinc-400 text-[11px]">Challenge:</span>{' '}
                      {asm.challenge}
                    </div>
                  )}

                  {asm.invalidationCondition && (
                    <div className="text-zinc-400 leading-relaxed font-mono text-[11px]">
                      <span className="text-zinc-400">Invalidation:</span>{' '}
                      <span className="text-zinc-300">{asm.invalidationCondition}</span>
                    </div>
                  )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 items-start">
          {stressScenarios.map((sc, idx) => {
            ...
            return (
              <div
                key={sc.id}
                className="p-5 rounded-2xl bg-zinc-900/40 border border-zinc-800/80 space-y-4 hover:border-zinc-700/80 transition-colors flex flex-col"
              >
                <div className="space-y-3.5">
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-2 pb-2.5 border-b border-zinc-800">
                    ...
                    <div className="flex items-center gap-1.5 font-mono text-[10px]">
                      ...
                      <span className="px-2 py-0.5 rounded bg-zinc-950 border border-zinc-800 text-zinc-300 font-medium">
                  ...
                  <div className="space-y-1 pt-1">
                    ...
                    <p className="text-xs text-zinc-200 font-medium leading-relaxed bg-zinc-950/60 border border-zinc-850 p-2.5 rounded-lg">
                  ...
                  {isExpanded && (
                    <div className="pt-3 border-t border-zinc-800/70 space-y-3 text-xs">
                      <div className="space-y-1">
                        <span className="font-mono text-[10px] text-zinc-400 uppercase tracking-wider block">
      ...
      <div className="p-6 rounded-2xl bg-zinc-900/40 border border-zinc-800/80 space-y-4">
        ...
        <div className="space-y-2">
          <p className="text-xs text-zinc-400 leading-relaxed">
            Observable market metrics established to trigger a formal reassessment of conviction. These represent research review thresholds, not automated trade orders.
          </p>
        ...
        {invalidationConditions.length > 1 && (
          <div className="pt-1">
            <button
              ...
              className="inline-flex items-center gap-1.5 text-xs font-mono text-zinc-400 hover:text-zinc-200 transition-colors p-1.5 -ml-1.5 rounded hover:bg-zinc-800/40 active:scale-[0.98]"
            >
              {showAllConditions ? (
                <>
                  <ChevronUp className="w-3.5 h-3.5" />
